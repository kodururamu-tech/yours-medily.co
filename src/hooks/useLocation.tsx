...
export const CITY_PRESETS = [
  { name: "Bengaluru", coords: { lat: 12.9716, lng: 77.5946 }, addresses: ["Indiranagar 100 Feet Rd", "Koramangala 4th Block", "Whitefield", "Jayanagar 3rd Block", "HSR Layout Sector 2", "Malleshwaram 15th Cross"] },
  { name: "Mumbai", coords: { lat: 19.0596, lng: 72.8295 }, addresses: ["Bandra West", "Andheri Link Road", "Colaba Causeway", "Juhu Tara Road", "Powai Central", "Dadar West"] },
  { name: "Delhi NCR", coords: { lat: 28.4595, lng: 77.0266 }, addresses: ["MG Road, Sector 14", "Connaught Place", "Saket Block A", "Vasant Kunj Phase 2", "Noida Sector 62", "Dwarka Sector 10"] },
  { name: "Kolkata", coords: { lat: 22.5726, lng: 88.3639 }, addresses: ["Park Street, Block B", "Lake View Road", "Salt Lake Sector V", "Gariahat Crossing", "Ballygunge Circular Rd", "New Town Action Area 1"] },
  { name: "Chennai", coords: { lat: 13.0012, lng: 80.2565 }, addresses: ["Adyar", "T. Nagar G.N. Chetty Rd", "Velachery Main Rd", "Mylapore High Rd", "Nungambakkam", "Anna Nagar West"] },
  { name: "Hyderabad", coords: { lat: 17.3850, lng: 78.4867 }, addresses: ["Gachibowli", "Jubilee Hills Rd 36", "Madhapur Hitec City", "Banjara Hills Rd 1", "Kondapur", "Secunderabad Club Rd"] },
  { name: "Pune", coords: { lat: 18.5204, lng: 73.8567 }, addresses: ["Koregaon Park Lane 6", "Kothrud Depot Rd", "Viman Nagar", "Hinjewadi Phase 1", "Baner Road", "Deccan Gymkhana"] }
];
...
export const LocationProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
      ...
      return new Promise<void>((resolve) => {
        navigator.geolocation.getCurrentPosition(
          async (position) => {
            ...
            try {
              const res = await fetch(`https://api.bigdatacloud.net/data/reverse-geocode-client?latitude=${newCoords.lat}&longitude=${newCoords.lng}&localityLanguage=en`);
              const data = await res.json();
  ...
  return (
    <LocationContext.Provider value={{ coords, cityName, locating, detectLocation, setManualLocation }}>
      {children}
